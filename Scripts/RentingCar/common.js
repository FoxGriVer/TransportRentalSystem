   ...
   viewChange : function (p, vw, dateInfo) {
    ...
    record : {
        addFromEventDetailsForm : function (win, rec) {
            Ext.net.DirectMethods.ShowMsg('Event ' + rec.data.Title + ' was added');
        },
        ...
        add : function (win, rec) {
            ...
            let outParams = {
                ...
                isAllDay: rec.data.IsAllDay,
                startDateTime: new Date(rec.data.StartDate),
                endDateTime: new Date(rec.data.EndDate),
                title: rec.data.Title
            }

            Ext.Ajax.request({
                url: "/Renting/SubmitDataFromCalendar/",
                method: 'post',
                params: outParams,
        ...
        updateFromEventDetailsForm : function (win, rec) {
            Ext.net.DirectMethods.ShowMsg('Event ' + rec.data.Title + ' was updated');
        },
        ...
        removeFromEventDetailsForm : function (win, rec) {
            Ext.net.DirectMethods.ShowMsg('Event ' + rec.data.Title + ' was deleted');
        },
        ...
        move : function (cal, rec) {
            rec.commit();
            Ext.net.DirectMethods.ShowMsg('Event '+ rec.data.Title + ' was moved to ' + Ext.Date.format(rec.data.StartDate, 'F jS' + (rec.data.IsAllDay ? '' : ' \\a\\t g:i a')));
        },
